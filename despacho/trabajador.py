...
def proceso_trabajador(id_trabajador, detener, listos, cola, resultados, cfg) -> None:
    ...
    hilos = [
        Despachador(id_trabajador, t, cola, resultados, detener, parar, log)
...
class Despachador(threading.Thread):
    """Consumidor: toma solicitudes de la cola, las despacha y registra la entrega."""

    def __init__(self, id_trabajador, idx, cola, resultados, detener, parar, log):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.detener.value:
                # Parada solicitada: la solicitud se retira de la cola sin atenderla y
                # se informa como cancelada para que el balance final cuadre.
                self.canceladas += 1
                self.resultados.put(Resultado(s.id, CANCELADA, self.id_trabajador, self.name,
                                              tid, s.t_llegada, t_inicio, t_inicio))
                continue

            self.log.info("DESPACHO solicitud %d (%s: %s -> %s) | esperó en cola %.0f ms",
                          s.id, s.cliente, s.origen, s.destino, (t_inicio - s.t_llegada) * 1000)
            time.sleep(s.t_despacho)        # preparación en el centro de despacho
            self.log.info("EN RUTA solicitud %d", s.id)
            time.sleep(s.t_entrega)         # trayecto hasta la entrega
            t_fin = time.monotonic()
            self.entregadas += 1
            self.log.info("ENTREGADA solicitud %d | servicio %.0f ms", s.id,
                          (t_fin - t_inicio) * 1000)
            self.resultados.put(Resultado(s.id, ENTREGADA, self.id_trabajador, self.name,
                                          tid, s.t_llegada, t_inicio, t_fin))
